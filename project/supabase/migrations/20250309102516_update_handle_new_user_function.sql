...
BEGIN
    ...
    IF default_role = 'provider' THEN
        INSERT INTO public.provider_profiles (id, user_id, full_name, bio)
        VALUES (gen_random_uuid(), NEW.id, user_full_name, user_bio);
        RAISE LOG 'Created provider profile for %', NEW.email;
    ELSIF default_role = 'admin' THEN
        INSERT INTO public.admin_profiles (id, user_id, full_name)
        VALUES (gen_random_uuid(), NEW.id, user_full_name);
